...
# Must match nih-sftp-server.c: MAX_ATTRS_BYTES and the NAME response header
# (SSH_FXP_NAME byte + id uint32 + count uint32) written before entries in
# sftp_readdir().
_MAX_ATTRS_BYTES = 32
_NAME_RESPONSE_HEADER = 1 + 4 + 4


def _readdir_calls_needed(total_entries, name_len):
    """How many READDIR calls it takes to drain a directory of
    `total_entries` entries (each with a name of `name_len` bytes) under
    the current build.MAX_PACKET, mirroring sftp_readdir()'s per-entry fit
    check. +1 call of margin for the "." and ".." entries, which are
    shorter and so don't perfectly divide out.
    """
    entry_bytes = (name_len + 4) * 2 + _MAX_ATTRS_BYTES
    entries_per_call = max(1, (build.MAX_PACKET - _NAME_RESPONSE_HEADER) // entry_bytes)
    return -(-total_entries // entries_per_call) + 1
...
class OpendirReaddirTest(SFTPTestCase):
    ...
    def test_readdir_with_many_entries_spans_multiple_calls(self):
        ...
        # Issue enough READDIR calls up front to drain the directory (each
        # call is independent given the server processes requests in order
        # within one session/process). How many calls that takes depends on
        # build.MAX_PACKET, since a smaller packet holds fewer entries per
        # response - see the entries-per-call math in _readdir_calls_needed.
        n_calls = _readdir_calls_needed(n_files + 2, name_len=len(f"f{n_files - 1:04d}.txt"))
