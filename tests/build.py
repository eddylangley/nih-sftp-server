...
# Must match the unguarded #define MAX_PACKET in nih-sftp-server.c.
DEFAULT_MAX_PACKET = 34000

# NIH_SFTP_MAX_PACKET overrides MAX_PACKET for every build variant, so tests
# whose expected values depend on it (e.g. test_protocol_hardening.py) must
# read MAX_PACKET below rather than hardcoding 34000.
MAX_PACKET = int(os.environ.get("NIH_SFTP_MAX_PACKET", DEFAULT_MAX_PACKET))
if MAX_PACKET != DEFAULT_MAX_PACKET:
    COMMON_FLAGS = [*COMMON_FLAGS, f"-DMAX_PACKET={MAX_PACKET}"]
